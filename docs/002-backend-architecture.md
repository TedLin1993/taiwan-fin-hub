...
使用預設排程（`schedule_mode = inherit`）的工作會加入唯一的未 claim 通知批次，不依賴工作實際完成後重新計算的 `next_run_at`。批次 header 與初始成員會以單一 D1 batch transaction 建立；`scheduled_sync_batch_results` 另保存成員建立時的 `scheduled_for` 快照。`sync_jobs.last_run_trigger` 用來區分 manual 與 scheduled 結果，成員 reconciliation 只有在 scheduled run 完成時才保留 `last_status`；手動成功或排程調整會略過該成員，手動失敗則等待真正的 scheduled retry。只有所有成員完成或被 reconciliation 略過時，scheduler 才以成員數量與 pending 狀態的條件式更新取得 claim 並送出一則彙總推播。自訂排程維持逐工作推播。
