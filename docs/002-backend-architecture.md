...
使用預設排程（`schedule_mode = inherit`）的工作會加入唯一的未 claim 通知批次，不依賴工作實際完成後重新計算的 `next_run_at`。批次 header 與初始成員會以單一 D1 batch transaction 建立；`scheduled_sync_batch_results` 另保存成員建立時的 `scheduled_for` 快照。成員 reconciliation 必須優先保留已完成的 `last_status`，只有停用、改為 custom 或明確調整排程且尚未完成的工作才會標記為 skipped。只有所有成員完成或被 reconciliation 略過時，scheduler 才以成員數量與 pending 狀態的條件式更新取得 claim 並送出一則彙總推播。自訂排程維持逐工作推播。
