...
| [`scheduled_sync_batches`](#scheduled_sync_batches) | 追蹤預設排程中需彙總推播的一輪同步工作。 | 6 | 0 | 0 |
...
> 注意：每個 batch_id 與 job_id 只保存一筆結果；scheduler 以結果數判斷整批是否完成。
...
| 2 | `job_id` | 已完成的同步工作識別碼。 | TEXT | NO | — | 2 | — |
| 3 | `connector_id` | 此次結果所屬的連接器。 | TEXT | NO | — | — | — |
| 4 | `status` | 工作結果，例如 success、failed 或 needs_user_action。 | TEXT | NO | — | — | — |
| 5 | `completed_at` | 工作結果寫入批次的時間。 | TEXT | NO | — | — | — |
...
> 注意：當結果數達 expected_jobs 時只允許一個 scheduler claim 推播，避免跨 Cron tick 重複通知。

#### Columns

| 順序 | 欄位 | 意義 | SQLite type | 可為 NULL | 預設值 | PK 順序 | Generated |
| ---: | --- | --- | --- | :---: | --- | ---: | --- |
| 1 | `id` | 批次識別碼，由 default 與預定執行時間組成。 | TEXT | YES | — | 1 | — |
| 2 | `scheduled_for` | 此輪預設排程原定執行時間。 | TEXT | NO | — | — | — |
| 3 | `expected_jobs` | 此輪預期完成的同步工作數。 | INTEGER | NO | — | — | — |
| 4 | `notification_claimed_at` | 彙總推播被 scheduler 取得發送權的時間。 | TEXT | YES | — | — | — |
| 5 | `created_at` | 批次建立時間。 | TEXT | NO | — | — | — |
| 6 | `updated_at` | 批次最後更新時間。 | TEXT | NO | — | — | — |
