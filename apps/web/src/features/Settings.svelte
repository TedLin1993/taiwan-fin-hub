<script lang="ts">
  ...
  import type { BankData, ClassificationRuleRow, ConnectorField, ConnectorId, MobileSettingsView, SyncJobRow, View } from "../lib/types";
  ...
  const sources: { id: ConnectorId; title: string; description: string }[] = [{ id: "einvoice", title: "電子發票", description: "財政部載具與品項明細" }, { id: "tdcc", title: "集保 e 存摺", description: "持倉、投資交易與銀行帳戶" }, { id: "esun", title: "玉山銀行", description: "帳戶、信用卡與交易" }, { id: "cathaybk", title: "國泰世華銀行", description: "帳戶與交易" }];
...
{:else}
  <div class="grid gap-5"><div class="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4"><Metric label="已設定來源" value="4" detail="資料連接器" /><Metric label="同步排程" value={String(enabledJobs)} detail="已啟用" /><Metric label="分類規則" value={String($rules.data?.length ?? 0)} detail="銀行交易" /><Metric label="需要處理" value={String(needsAction)} detail="同步狀態" tone={needsAction ? "negative" : "positive"} /></div><section class="grid gap-3 md:grid-cols-2">{#each sources as source}<SourceCard {api} {...source} id={source.id} jobs={$jobs.data ?? []} selected={selectedConnector === source.id} onConfigure={() => selectConnector(source.id)} />{/each}</section>{#if selectedConnector}{#key selectedConnector}<ConnectorPanel {api} connectorId={selectedConnector} demoMode={demoMode} title={sources.find((s) => s.id === selectedConnector)?.title ?? selectedConnector} fields={connectorFields[selectedConnector]} />{/key}{/if}<section class="grid gap-5 xl:grid-cols-2"><ExchangeRatesPanel {api} /><ClassificationRulesPanel {api} /></section></div>
{/if}
