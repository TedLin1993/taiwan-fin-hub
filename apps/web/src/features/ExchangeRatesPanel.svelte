...
<Card><CardHeader><h2 class="text-lg font-semibold">匯率</h2><p class="text-xs text-ink/45">管理外幣換算使用的參考匯率</p></CardHeader><CardContent><div class="grid gap-3">{#if currencies.length === 0}<p class="text-sm text-ink/50">目前沒有外幣帳戶。</p>{:else}{#each currencies as currency}<label class="flex items-center justify-between gap-3 text-sm"><span class="font-semibold">{currency}</span><input class="w-32 rounded-md border border-ink/15 bg-paper px-3 py-1.5 text-right text-sm outline-hidden" type="number" step="0.0001" bind:value={values[currency]} /></label>{/each}<Button disabled={$save.isPending} onclick={() => $save.mutate(Object.fromEntries(Object.entries(values).map(([k, v]) => [k, Number(v)])))}><Save class="size-4" />{$save.isPending ? "儲存中…" : "儲存匯率"}</Button>{/if}</div></CardContent></Card>
