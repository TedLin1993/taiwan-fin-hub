...
{:else}
  <div class="grid gap-5">
                    ...
                    {#if $history.isPending}<p class="mt-3 text-sm text-ink/45">載入歷史中…</p>{:else if ($history.data ?? []).length === 0}<p class="mt-3 text-sm text-ink/45">尚無歷史紀錄。</p>{:else}
                      <div class="mt-2 divide-y divide-ink/8">
                        {#each $history.data ?? [] as entry}
                          <div class="flex items-center justify-between gap-3 py-2 text-sm"><span>{formatDate(entry.date)}</span>{#if editingHistoryDate === entry.date}<div class="flex items-center gap-2"><input class="w-28 rounded-md border border-ink/15 bg-white px-2 py-1" type="number" bind:value={editingHistoryValue} /><button class="text-steel" onclick={() => $editHistory.mutate({ assetId: asset.id, value: Number(editingHistoryValue), date: entry.date })}>儲存</button><button class="text-ink/50" onclick={() => editingHistoryDate = null}>取消</button></div>{:else}<div class="flex items-center gap-2"><span class="font-medium">{formatCurrency(entry.value)}</span><button class="text-xs text-steel" onclick={() => { editingHistoryDate = entry.date; editingHistoryValue = String(entry.value); }}>編輯</button><button class="text-xs text-coral" onclick={() => $deleteHistory.mutate({ assetId: asset.id, date: entry.date })}>刪除</button></div>{/if}</div>
                        {/each}
                      </div>
                    {/if}
                    <div class="mt-3 flex flex-wrap items-end gap-2 border-t border-ink/8 pt-3"><label class="grid gap-1 text-xs text-ink/55">估值<input class="w-32 rounded-md border border-ink/15 bg-white px-2 py-1.5 text-sm" type="number" bind:value={historyValue} /></label><label class="grid gap-1 text-xs text-ink/55">日期<input class="rounded-md border border-ink/15 bg-white px-2 py-1.5 text-sm" type="date" bind:value={historyDate} /></label><Button size="sm" disabled={!historyValue || $addHistory.isPending} onclick={() => $addHistory.mutate({ assetId: asset.id, value: Number(historyValue), date: historyDate })}><Plus class="size-4" />新增估值</Button></div>
                  </div>
                {/if}
              </div>
            {/each}
    ...
    {#if adding || editing}<div class="fixed inset-0 z-[70] flex items-end bg-ink/45 md:items-center md:justify-center md:p-6"><div class="w-full rounded-t-2xl bg-white p-5 shadow-2xl md:max-w-lg md:rounded-2xl"><div class="flex items-center justify-between"><h2 class="text-xl font-semibold">{editing ? "編輯資產" : "新增資產"}</h2><button class="size-10 rounded-full bg-ink/5 text-xl" onclick={() => { adding = false; editing = null; }}>×</button></div><div class="mt-5 grid gap-3"><label class="grid gap-1 text-sm">名稱<input class="rounded-md border border-ink/15 bg-paper px-3 py-2 text-sm outline-hidden" bind:value={form.name} /></label><label class="grid gap-1 text-sm">類別<select class="rounded-md border border-ink/15 bg-paper px-3 py-2 text-sm outline-hidden" bind:value={form.category}>{#each Object.entries(categories) as [key, label]}<option value={key}>{label}</option>{/each}</select></label><label class="grid gap-1 text-sm">目前估值<input type="number" class="rounded-md border border-ink/15 bg-paper px-3 py-2 text-sm outline-hidden" bind:value={form.value} /></label><label class="grid gap-1 text-sm">估值日期<input type="date" class="rounded-md border border-ink/15 bg-paper px-3 py-2 text-sm outline-hidden" bind:value={form.date} /></label><label class="grid gap-1 text-sm">備註<textarea class="rounded-md border border-ink/15 bg-paper px-3 py-2 text-sm outline-hidden" rows="2" bind:value={form.note}></textarea></label></div><div class="mt-5 grid grid-cols-2 gap-3"><Button variant="secondary" onclick={() => { adding = false; editing = null; }}>取消</Button><Button disabled={$add.isPending || $update.isPending} onclick={submit}>{$add.isPending || $update.isPending ? "儲存中…" : "儲存"}</Button></div></div></div>{/if}
  </div>
{/if}
