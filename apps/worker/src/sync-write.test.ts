...
class SqliteStatement {
  ...
  execute() {
    ...
    const result = this.owner.database.prepare(this.sql).run(...this.values as never[]);
...
describe("staged sync persistence", () => {
  it("stages records in bounded JSON chunks and advances the cursor only after promotion", async () => {
    const db = createDb();
    const records = Array.from({ length: 205 }, (_, index) => bankAccountRecord(index));
    records.push({
    ...
    await persistStagedSyncWrite(db as unknown as D1Database, {
      records,
      finalizeStatements: [
        db.prepare("UPDATE connector_settings SET sync_cursor = ? WHERE connector_id = ?")
          .bind("new-cursor", "tdcc") as unknown as D1PreparedStatement
      ]
    });

    expect(db.executedSql.filter((sql) => sql.includes("INSERT INTO sync_write_staging"))).toHaveLength(3);
    expect(db.database.prepare("SELECT COUNT(*) AS count FROM bank_accounts").get()).toMatchObject({ count: 205 });
    expect(db.database.prepare("SELECT effective_date AS effectiveDate FROM bank_transactions").get())
      .toMatchObject({ effectiveDate: "2026-07-19" });
    expect(db.database.prepare("SELECT COUNT(*) AS count FROM sync_write_staging").get()).toMatchObject({ count: 0 });
    expect(db.database.prepare("SELECT sync_cursor AS cursor FROM connector_settings WHERE connector_id = 'tdcc'").get())
  ...
  it("rolls back promotion and leaves the cursor unchanged when a staged record is invalid", async () => {
    ...
    await expect(persistStagedSyncWrite(db as unknown as D1Database, {
    ...
    expect(db.database.prepare("SELECT COUNT(*) AS count FROM bank_transactions").get()).toMatchObject({ count: 0 });
    expect(db.database.prepare("SELECT COUNT(*) AS count FROM sync_write_staging").get()).toMatchObject({ count: 0 });
    expect(db.database.prepare("SELECT sync_cursor AS cursor FROM connector_settings WHERE connector_id = 'tdcc'").get())
