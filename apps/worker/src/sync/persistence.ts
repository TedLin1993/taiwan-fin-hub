...
const ENTITY_CONFIG: Record<SyncEntityType, EntityConfig> = {
  invoice: {
    table: "invoices",
    columns: [
      "id", "connector_id", "source_id", "invoice_number", "invoice_date", "seller_name",
      "amount", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "source_id"],
    updateColumns: [
      "invoice_number", "invoice_date", "seller_name", "amount", "raw_payload", "updated_at"
    ]
  },
  invoice_line_item: {
    table: "invoice_line_items",
    columns: [
      "id", "invoice_id", "connector_id", "invoice_source_id", "source_id", "line_number",
      "description", "quantity", "unit_price", "amount", "raw_payload", "created_at", "updated_at"
    ],
  ...
  bank_account: {
    ...
    columns: [
      ...
      "currency", "credit_limit", "bank_code", "account_last4", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "source_id"],
    updateColumns: [
      "institution_name", "account_name", "account_type", "currency", "credit_limit", "bank_code",
      "account_last4", "raw_payload", "updated_at"
    ]
  },
  bank_balance_snapshot: {
    ...
    columns: [
      ...
      "statement_balance", "payment_due_date", "statement_closing_date", "no_payment_needed",
      "currency", "as_of_at", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "account_id", "source_id"],
    updateColumns: [
      "balance", "available_balance", "statement_balance", "payment_due_date", "statement_closing_date",
      "no_payment_needed", "currency", "as_of_at", "raw_payload", "updated_at"
    ]
  },
  bank_transaction: {
    ...
    columns: [
      ...
      "currency", "description", "counterparty", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "account_id", "source_id"],
    updateColumns: [
      "posted_date", "authorized_at", "amount", "currency", "description", "counterparty",
      "raw_payload", "updated_at"
    ]
  },
  credit_card_bill: {
    ...
    columns: [
      ...
      "minimum_payment", "paid_amount", "is_paid", "payment_due_date", "statement_closing_date",
      "currency", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "account_id", "billing_period"],
    updateColumns: [
      "source_id", "statement_amount", "minimum_payment", "paid_amount", "is_paid", "payment_due_date",
      "statement_closing_date", "currency", "raw_payload", "updated_at"
    ]
  },
  investment_position: {
    ...
    updateColumns: [
      "asset_type", "symbol", "name", "quantity", "market_value", "cash_balance", "currency",
      "raw_payload", "updated_at"
    ]
  },
  investment_transaction: {
    ...
    columns: [
      ...
      "transaction_name", "quantity", "price", "amount", "currency", "raw_payload", "created_at", "updated_at"
    ],
    conflictColumns: ["connector_id", "account_id", "source_id"],
    updateColumns: [
      "broker_no", "broker_account", "broker_name", "symbol", "name", "asset_type", "trade_date",
      "posted_date", "transaction_code", "transaction_name", "quantity", "price", "amount", "currency",
      "raw_payload", "updated_at"
    ]
  },
  net_worth_history: {
    table: "net_worth_history",
    columns: ["id", "date", "net_worth", "asset_type", "source", "snapshotted_at"],
...
export async function persistStagedSyncWrite(
  ...
  await db.prepare("DELETE FROM sync_write_staging WHERE created_at < ?")
    .bind(new Date(Date.now() - STAGING_RETENTION_MS).toISOString())
    .run();

  try {
    for (let offset = 0; offset < input.records.length; offset += STAGING_CHUNK_SIZE) {
      ...
      ).bind(runId, createdAt, JSON.stringify(chunk)).run();
    }

    const entityTypes = new Set(input.records.map((record) => record.entityType));
    await db.batch([
      ...(input.beforePromoteStatements ?? []),
      ...ENTITY_ORDER
        .filter((entityType) => entityTypes.has(entityType))
    ...
    await db.prepare("DELETE FROM sync_write_staging WHERE run_id = ?")
      .bind(runId)
      .run()
      .catch(() => undefined);
    throw error;
...
function promotionStatement(db: D1Database, runId: string, entityType: SyncEntityType) {
